...
impl StateDB {
    ...
    pub fn store_state(&self, hash: &str, state: &StoredState) -> Result<()> {
        let cf = self.db.cf_handle("states")
            .ok_or_else(|| StorageError::InvalidData("states column family not found".to_string()))?;
        ...
        self.db.put_cf(cf, timestamp_key.as_bytes(), hash.as_bytes())?;
    ...
    pub fn store_signed_state(&self, hash: &str, signed_state: &SignedStoredState) -> Result<()> {
        let cf = self.db.cf_handle("signatures")
            .ok_or_else(|| StorageError::InvalidData("signatures column family not found".to_string()))?;
    ...
    pub fn get_compilation_stats(&self) -> Result<CompilationStats> {
        ...
        let mut target_models: HashMap<String, u32> = HashMap::new();
