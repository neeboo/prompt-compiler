...
use anyhow::Result;
use prompt_compiler_core::{PromptCompiler, ModelTarget, PromptIR, ContextEntry};
use prompt_compiler_core::compiler::analyzers::{SemanticAnalyzer, ContextLearningAnalyzer};
use prompt_compiler_core::compiler::optimizers::{WeightUpdateOptimizer, TokenBudgetOptimizer, PriorityBalanceOptimizer};
use prompt_compiler_core::compiler::generators::{StandardPromptGenerator, WeightAwareGenerator};
use prompt_compiler_weights::{ImplicitDynamics, DynamicsConfig, create_random_weights, create_random_vector};
use colored::*;
...
impl Cli {
    pub async fn execute(self) -> Result<()> {
        match self.command {
            Commands::Compile { 
                ...
                self.handle_compile(prompt, model, budget, priority, enable_weight_analysis, format).await
            },
            Commands::Analyze { prompt, analyzer } => {
                self.handle_analyze(prompt, analyzer).await
            },
            Commands::Optimize { prompt, optimizer, budget } => {
                self.handle_optimize(prompt, optimizer, budget).await
            },
            Commands::WeightDemo { context_count, verbose } => {
    ...
    async fn handle_analyze(&self, prompt: String, analyzer_type: String) -> Result<()> {
    ...
    async fn handle_optimize(&self, prompt: String, optimizer_type: String, budget: Option<u32>) -> Result<()> {
    ...
    async fn handle_weight_demo(&self, context_count: usize, verbose: bool) -> Result<()> {
        println!("{}", "🧠 Weight Update Dynamics Demo".magenta().bold());
        println!("Based on: Learning without training: The implicit dynamics of in-context learning\n");
        
        // Create random weights and context
        let weights = create_random_weights(8, 8);
        ...
        for (i, update) in updates.iter().enumerate() {
            println!("Step {}: Update norm = {:.4}, Effectiveness = {:.4}", 
                i + 1, 
                update.delta_w.norm(),
            ...
            if verbose {
                println!("   Context vector norm: {:.4}", update.context_vector.norm());
        ...
        println!("Converged: {}", if convergence.is_converged { "Yes".green() } else { "No".yellow() });
        ...
        println!("\n{}", "💡 This demonstrates the implicit weight update mechanism described in the paper".blue().italic());

        Ok(())
    }

    async fn handle_version(&self) -> Result<()> {
        println!("{} {}", "Prompt Compiler".cyan().bold(), env!("CARGO_PKG_VERSION").green());
        println!("Based on: Learning without training: The implicit dynamics of in-context learning");
